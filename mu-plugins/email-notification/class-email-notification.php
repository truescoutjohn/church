...
class Email_Notification {
    ...
    public function send_email($to, $subject, $message, $type, $headers = [], $attachments = []) {
        $job = [
            'type' => $type,
            'to' => $to,
            'subject' => $subject,
        ...
        return $this->queue->push('emails', json_encode($job));
    }

    public function remove_email_from_queue($key){
        return $this->queue->pop($key);
    }

    public function get_email_count(){
        return $this->queue->size('emails');
    }
}
//Обработчик очереди
// class Email_Worker {
